---
title: "<Project Name> Claude Context Rules"
type: context_rules
visibility: internal
completion: 100
status: ready
...
# `<Project Name>` — Claude Context Rules

This file customizes how Claude (or any LLM agent) operates inside the `<Project Name>` wiki. It is read on session start and supplements any global Claude Code or chat configuration with project-specific guidance.
...
Briefly describe what `<Project Name>` is, who it serves, and what makes it distinctive. Two or three sentences are enough. The goal is to anchor an agent that has just opened this wiki for the first time.
...
When answering any question about `<Project Name>` — whether posed by a human collaborator, a downstream agent, or as part of a generated document — follow these four rules. They are the minimum required behavior for every bootstrapped wiki and must remain in this file.
