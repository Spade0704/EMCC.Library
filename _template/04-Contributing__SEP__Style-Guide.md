...
Conventions for authoring pages in the `<Project Name>` wiki. The goal is consistency that lets validators and human readers parse pages quickly without surprises.
...
- **`type`** — One of: `framework | reference | guide | overview | episode | dashboard | profile | brain_dump | decision | context_rules`. Pick the closest match; see [[File-Routing]] for type-to-folder mapping.
...
- **`last_updated`** — `YYYY-MM-DD`. Bump on every substantive edit so cascade-staleness detection (see [[Update-Cascade]]) stays accurate.
...
- **`allow_forbidden_terms: true`** — Escape hatch for pages that intentionally include a term that `_config/forbidden_terms.yaml` would otherwise flag. Use sparingly and explain why in the page body. See [[Terminology-Rules]].
...
- **Wikilinks** — Use `[[Page-Name]]` (bare stem; the canonical form). The resolver looks up targets by filename stem only — folder-prefixed forms like `[[Folder/Page-Name]]` are not resolved by the canonical resolver and produce broken-ref warnings. Stems must be unique wiki-wide; rename if collision.
...
Stay inside the vocabulary documented in [[Glossary]] and the rules in [[Terminology-Rules]]. The terminology validator (`_scripts/validate_terminology.py`) reads `_config/forbidden_terms.yaml`; the reveal-leak validator (`_scripts/validate_reveal_conceit.py`) reads `_config/reveal_leak_patterns.yaml`.
