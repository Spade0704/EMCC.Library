...
| `personas/CLAUDE.librarian.md` | Librarian persona (canonical) | Persona / agent work |
| `personas/CLAUDE.auditor.md` | Auditor persona (carried verbatim from DFDU) | On-demand audit work; Lattice 3.0 Regime B |
...
- **Lattice 3.0 protocol governs Library's own sessions.** When Library work is Level-2+ (cross-module touches, spec changes, etc.), Library boots a Lattice 3.0 session per the `LATTICE_PROFILE` block above. Architect plan + Auditor verdict required. Persona files in `personas/`.
