...
Dedicated-page detection per spec line 444 placeholder ('current
placeholder is a filename match against the slugified canonical
name') — Path A heuristic: slug = canonical_name.lower().replace(' ',
'-'); page with that filename stem (any content folder) suppresses
the finding.
...
def run(wiki_root: Path) -> Dict[str, Any]:
    ...
    dedicated_stems: Set[str] = {p.stem.lower() for p in all_pages}
    ...
    content = _build_dashboard_markdown(
        findings, len(roster), len(scan_pages)
    )
