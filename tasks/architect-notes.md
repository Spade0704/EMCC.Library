...
2. Future: an **`emcc-codex`** marketplace plugin for Librarian skills (Ingest / Semantic-Lint / Maintain), mirroring `emcc-lattice` — factor prompts from `CODEX_LIBRARIAN.md`. ~~Open question: do we want a drift guard (`claude-library.md` ⇄ `CODEX_LIBRARIAN.md`) analogous to `generate_persona_dropin.py`'s `--check`?~~ **Answered — NO; see fork (b) RESOLVED-STOP below.**
...
**Decision forks:** (a) ~~Librarian skill names + ship `emcc-codex` now vs later~~ **RESOLVED 2026-05-28** — shipped now as `/ingest` `/lint` `/maintain` `/sync` (skills load persona + verbatim procedure; no spec change). (b) ~~drift-guard for the vendored `claude-library.md` ⇄ `CODEX_LIBRARIAN.md`~~ **RESOLVED 2026-06-19 — STOP / will NOT build** (pre-build gate, Regime-B judgment; Director cascade dir-20260619). Rationale: (1) **Wrong pattern — not actually analogous.** `generate_persona_dropin.py`'s `--check` works only because the persona drop-in IS the canonical body *verbatim* (frontmatter + DO-NOT-EDIT banner + `CODEX_LIBRARIAN.md` body) — a pure regenerate-and-byte-compare. `claude-library.md` is a 25-line hand-authored *pointer/summary* with `{{PROJECT_NAME}}`/`{{WIKINAME}}` placeholders that POINTS at the canonical (names paths, op/skill names, a hard-rules digest, "Full set in `CODEX_LIBRARIAN.md`"); there is no deterministic `f(CODEX_LIBRARIAN.md) → claude-library.md`. Per `tasks/lessons.md` "deterministic generation is a prerequisite for drift guards" — a curated summary is not a derivation, so regenerate-and-compare cannot apply; the only alternative (a fuzzy semantic checker) is fragile/false-positive-prone and is NOT "analogous to generate_persona_dropin.py". (2) **Wrong repo.** The template is owned by EMCC (`EMCC/templates/shared/module-templates/claude-library.md`), not Library; any guard belongs in EMCC's suite, not here. (3) **YAGNI / one consumer.** Modular layout live in iSommelier only (pilot); tat_app + supplystationusa still pending — ~1 consumer today. (4) **Low drift surface by design.** The pointer defers all detail to the verbatim-synced `_context/CODEX_LIBRARIAN.md` (already an OVERWRITE-lane sync guard); it changes only on a path/op/skill *rename*, which a content-drift guard wouldn't even catch better than a broken-link lint. If real need emerges (3+ modular consumers AND observed pointer staleness), revisit as an EMCC-side reference-integrity lint (do the named paths/skills resolve?), NOT a generate-and-compare guard. (c) whether `bootstrap.py` should emit the modular `.claude/modules/` layout for greenfield wikis directly (today `emcc_wire.py` adds it post-hoc) — still open.
